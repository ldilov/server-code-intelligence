...
export function toRelativePath(rootPath, targetPath) {
    return path.relative(rootPath, targetPath).replace(/\\/g, "/");
}
export function ensureWithinRoot(rootPath, targetPath) {
    const normalizedRoot = normalizePath(rootPath);
    const normalizedTarget = normalizePath(targetPath);
    return normalizedTarget === normalizedRoot || normalizedTarget.startsWith(`${normalizedRoot}/`);
}
export function pathToModuleLanguage(filePath) {
    const extension = extensionOf(filePath);
    return defaultLanguageByExtension[extension] ?? extension.replace(/^\./, "") || "unknown";
}
export function resolveLocalModulePath(fromFilePath, importText, candidateExtensions = defaultCandidateExtensions) {
