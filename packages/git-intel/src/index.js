...
const defaultGitExecutor = (rootPath, args) => execFileSync("git", ["-C", rootPath, ...args], {
    encoding: "utf8",
...
export function parseRecentCommits(output, workspaceId, repoId, branchName) {
    const now = nowIso();
    return output
        .split("\u001e")
...
export class GitIntelCollector {
    execute;
    constructor(execute = defaultGitExecutor) {
        this.execute = execute;
    }
    collect(rootPath, workspaceId, repoId, commitLimit = 5) {
        try {
            ...
            const recentCommits = parseRecentCommits(this.execute(rootPath, ["log", `--max-count=${commitLimit}`, "--date=iso-strict", "--pretty=format:%H%x1f%an%x1f%aI%x1f%s%x1e"]), workspaceId, repoId, branchName || undefined);
