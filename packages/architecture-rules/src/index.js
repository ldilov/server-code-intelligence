...
import { defaultArchitectureConfigCandidates, defaultGeneratedPatterns, globToRegExp, normalizePath, nowIso, stableId, toRelativePath } from "@local-engineering-brain/shared-utils";
    ...
    for (const rawLine of lines) {
        ...
        if (indent === 2 && line.startsWith("- ")) {
            ...
            if (inline.length > 0 && inline.includes(":")) {
                ...
                const key = inline.slice(0, separatorIndex).trim();
...
function isPathMatch(pattern, candidate) {
    return globToRegExp(pattern).test(candidate);
}
function isGenerated(config, relativePath) {
    return (config.generatedPatterns ?? []).some((pattern) => isPathMatch(pattern, relativePath));
}
function isExceptionMatch(rule, sourceRelativePath, targetRelativePath) {
...
function isRuleMatch(rule, sourceRelativePath, targetRelativePath) {
    return isPathMatch(rule.from, sourceRelativePath) && isPathMatch(rule.to, targetRelativePath);
}
export async function loadArchitectureRules(workspaceRoot) {
...
export function evaluateArchitectureRules(input) {
