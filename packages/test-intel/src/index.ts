...
import type { EdgeRecord, ExtractedTestFact, ParserContext, RangePosition, TestFramework } from "@local-engineering-brain/core-types";
...
function firstArgumentText(node: ts.CallExpression): string | undefined {
...
function detectFramework(sourceText: string, sourceFile: ts.SourceFile): TestFramework {
...
export class TestIntelExtractor {
  public extract(context: ParserContext, sourceText: string): ExtractedTestFact | undefined {
    ...
    const cases: ExtractedTestFact["testCases"] = [];
    ...
    const visit = (node: ts.Node, suiteStack: string[]): void => {
    ...
    if (cases.length === 0) {
      const start: RangePosition = { line: 1, column: 1 };
